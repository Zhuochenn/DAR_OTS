...
class I2FPDataset(Dataset):
    ...
    def __init__(self, img_path, fp_path, rfimage, mean_std):
        ...
        self.force_label = pd.read_csv(fp_path)
        ...
        self.force_label['Fx(N)'] = (self.force_label['Fx(N)'] - mean_std[0]) /mean_std[3]
        self.force_label['Fy(N)'] = (self.force_label['Fy(N)'] - mean_std[1]) /mean_std[4]
        self.force_label['Fz(N)'] = (self.force_label['Fz(N)'] - mean_std[2]) /mean_std[5]
    
    def __len__(self):
        return len(self.force_label)
    ...
    def __getitem__(self, idx):
        ...
        label = torch.tensor(self.force_label.iloc[idx, :].values, dtype=torch.float32)
        if self.transform:
