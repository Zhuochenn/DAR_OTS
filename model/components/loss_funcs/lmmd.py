from model.components.loss_funcs.mmd import MMDLoss
from model.components.loss_funcs.adv import LambdaSheduler
import torch
import numpy as np
...
class LMMDLoss(MMDLoss, LambdaSheduler):
    ...
    def cal_weight(self, source_label, target_logits):
        ...
        source_label_sum[source_label_sum == 0] = 100  # avoid denominator is 0
        source_label_onehot = source_label_onehot / source_label_sum # source label ratio in one batch
        ...
        target_logits = target_logits / target_logits_sum  # target label ratio in one batch
        ...
        for i in range(self.num_class): # (B, C)
            if i in set_s and i in set_t:
                ...
                weight_ss = weight_ss + ss    # batch_size * batch_size
